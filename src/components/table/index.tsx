...
export default {
      ...
      >
        <div class="table-scroll">
          <div
            ...
            style={{ overflow: "scroll", marginBottom: `-${this.scrollBarWidth}px` }}
            ref="h-table-scroll-head"
            vOn:scroll_self={e => this.normalScroll(e)}
          >
            <table style={{ width: this.scroll.x ? this.scroll.x + 'px' : '100%' }}>
              <colgroup>
                {this.rowSelection.type && <col class="hss-table-selection-col" />}
              ...
              <thead class="hss-table-thead">
                <tr>
                  <th>
                    <div class="hss-checkbox" vOn:click={() => this.onSelectAll()}>
                      <input
                        type="checkbox"
                        class={{
                  ...
                  >
                    <td>
                      <div
                        class={{
                          'hss-checkbox': true,
                          'hss-checkbox-disabled': this.rowSelection.getCheckboxProps(rowItem).disabled
                        }}
                        vOn:click={e => this.onSelect(rowItem, this.isSelected(rowItem), e)}
                      >
                        <input
                          type="checkbox"
                          class={{
                          ...
                          disabled={this.rowSelection.getCheckboxProps(rowItem).disabled}
                          value={rowItem}
                    ...
                    {this.columns.map((columnsItem, columnsIndex) => {
                      return <td
                        ...
                        style={{ 'text-align': columnsItem.align ? columnsItem.align : 'left' }}
                      >
                        {
        ...
        {this.fixedLeftData.length && <div class="fixed-left">
          <div>
                ...
                <tr>
                    ...
                    <div class="hss-checkbox" vOn:click={() => this.onSelectAll()}>
                      ...
                      <span
                        class={{
                          'hss-checkbox-inner': true,
                          'none-selected': this.selectedList.length - this.intersection.length == 0,
                          'no-all': !(this.selectedList.length - this.intersection.length == 0) &&
                  ...
                  {this.fixedLeftData.map((item, index) => {
                    return (<th
                      key={index}
                      column-key={item.column.key || index}
                      style={{ 'text-align': item.column.col.align ? item.column.col.align : 'left' }}
                    >
                ...
                <tbody class="hss-table-tbody">
                  {this.sourceData.map((item, index) => {
                    return <tr
                      ...
                      vOn:mouseleave={e => this.mouseLeave()}
                    >
                      <td>
                        <div
                          class={{
                            'hss-checkbox': true,
                            'hss-checkbox-disabled': this.rowSelection.getCheckboxProps(this.fixedLeftData[0].data[index]).disabled
                          }}
                          vOn:click={e => this.onSelect(this.fixedLeftData[0].data[index], this.isSelected(this.fixedLeftData[0].data[index]), e)}
                        >
                          <input
                            type="checkbox"
                            class={{
                              'hss-checkbox-input': true,
                              'hss-checkbox-checked': this.isSelected(this.fixedLeftData[0].data[index]),
                              'hss-checkbox-disabled': this.rowSelection.getCheckboxProps(this.fixedLeftData[0].data[index]).disabled
                            }}
                            disabled={this.rowSelection.getCheckboxProps(this.fixedLeftData[0].data[index]).disabled}
                      ...
                      {this.fixedLeftData.map((col, colIndex) => {
                        ...
                        >
                          ...
                          {typeof col.column.col.render == 'function' ?
                            this.$slots[`fixed-left-${col.column.col.dataIndex}-${colIndex}-${index}`] :
        ...
        {this.fixedRightData.length && <div class="fixed-right">
          <div>
            <table style="min-width:auto">
              <colgroup>
                  ...
                  >
                    {this.fixedRightData.map((col, colIndex) => {
                      ...
                      >
                        ...
                        {typeof col.column.col.render == 'function' ? this.$slots[`fixed-right-${col.column.col.key}-${colIndex}-${index}`] : this.getRowData(col.column.col, col.data[index])}
                      </td>
                    })}
                  </tr>
                })}
  ...
  watch: {
    selectedList(newVal, oldVal) {
      ...
      newVal.length == this.sourceData.length && (this.tableIsSelectAll = true);
      // let key = this.getRowKey()
      let newSelectedRowKeys = newVal.map(v => {
        // console.log(v, "vv");
        return v.key;
      });
  ...
  created() {
    ...
    this.columns.map(item => {
      // console.log(item);
      //判断columnKey是否正确
      let columnKey = this.getColumnKey(item);
      if (!columnKey) {
        console.error(
          "columnKey有误（column中key以及dataIndex均无）,将使用v-for遍历时的index作为columnKey"
        );
      }
      let fixed = item.fixed ? item.fixed : false;
      if (item.fixed === true || fixed == "left") {
        let fixedLeftRowData = this.sourceData.filter(v => {
          // row的key优先级：rowKey > key
          let rowKey = this.getRowKey(v);
          if (!rowKey) {
            console.error(
        ...
        let fixedRightRowData = this.sourceData.filter(v => {
          let rowKey = this.getRowKey(v);
          if (!rowKey) {
            console.error(
              "请确保dataSource中的每项数据都有key值，如果没有，请指定rowKey！"
        ...
        let normalRowData = this.sourceData.filter(v => {
  ...
  methods: {
    ...
    onSelectAll(v) {
      ...
      if (this.tableIsSelectAll) {
        ...
        let changeData = this.sourceData.filter(item => {
        ...
        this.selectedList = this.sourceData.filter(v => {
