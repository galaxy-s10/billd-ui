 ...
 * @param options
 */
function handleLineData(options) {
  var data;
  ...
  if (options.groupDimension && options.groupDimension.field && options.metric.length) {
    ...
    options.data.forEach(function (item) {
      ...
      minMetric = Math.min(minMetric, item[options.metric[0].uid] || 0); // 计算数值总和
    ...
    data = dimensionArray.map(function (key, rowIndex) {
      ...
      if (options.metricRange) {
        ...
        Object.values(_extends({}, group, fields["tmp_" + key])).forEach(function (metric, index) {
          metricRangeData[rowIndex][index] = metric;
        });
    ...
    options.data.forEach(function (item, rowIndex) {
      ...
      _dimension = "tmp" + item[options.dimension.uid];
