 ...
 * 排序实例
 */
var ReportSortModule = /*#__PURE__*/function (_FieldMapping) {
  _inherits(ReportSortModule, _FieldMapping);

  var _super = _createSuper(ReportSortModule);

  function ReportSortModule(chartData) {
    var _this;
    ...
    _this.title = '排序';
    _this.max = 0;
    _this.moduleTypes = ["dimension", "groupDimension", "metric"];
    return _this;
  }
  /**
   * 处理图表数据
    ...
    value: function handleChartData(chart) {
      ...
      if (chartData && !chartData.sort) {
        ...
        if (chartData.metricGroup && chartData.metricGroup.length > 0) {
          var metrics = [];
          chartData.metricGroup.forEach(function (m) {
            metrics.push.apply(metrics, _toConsumableArray(m));
          });
          fieldCol = fieldCol.concat(metrics);
        }

        chartData.sort = chartData.sort.filter(function (field) {
          return fieldCol.findIndex(function (item) {
            return item.uid === field.uid;
          }) > -1;
        });
      }
    }
...
exports.default = ReportSortModule;